...
- JSON 字段：camelCase
- null 值：保留（不省略键）
- 快照中 `documents` 与 `relations` 的稳定排序必须使用 `<` / `>` 等显式二进制字符串比较，禁止依赖默认 `localeCompare()`；测试至少覆盖大小写、数字段和非 ASCII 路径/ID，避免不同 Node / ICU / locale 环境制造无意义 git diff
- 快照写入必须使用目标文件同目录临时文件 + 原子 `rename()`；临时写失败时必须保留既有快照并 best-effort 清理临时文件；已存在输出文件允许被原子替换，禁止用“拒绝覆盖”改变导出刷新语义
- CLI `--output` 若以路径分隔符结尾（如 `snapshots/`），必须解析为目录下的默认 `cord-snapshot.json`
- CLI 输出路径必须在 `serviceFactory()` 前完成 project-root 词法边界校验，覆盖 POSIX `../...`、win32 跨盘符、win32 UNC 跨 root；若 `relative(projectRoot, input)` 结果仍是绝对路径，必须视为 project-root 外并返回稳定 `ConfigError`
- 对已存在的输出祖先目录，CLI 还必须通过 `realpath` 校验物理边界，拒绝 symlink 指向 project-root 外的输出位置；回归测试至少覆盖 win32 UNC projectRoot 内部路径、目录形态输出和 symlink 物理逃逸
...
**P45. 框架/BMAD 文件发现的局部 fs 异常必须非致命（CR-SCAN-04）：**
...
**P46. Markdown 链接规则必须通用过滤非文件 URI scheme（CR-SCAN-05）：**
...
**P47. BMAD frontmatter 检测只接受独立分隔符行（CR-SCAN-06）：**
...
**P48. BMAD frontmatter 候选必须优先扫描高价值路径（CR-SCAN-07）：**
...
- `--verbose` / debug 级别必须在 async action 执行前生效；若通过 root-level `-v` / `--verbose` 开启 debug，入口层必须在 `parseAsync` 前预处理或使用可靠的 pre-action 机制，并用真实 Commander action 内 `logger.debug()` 回归测试锁定（CR-CLI-05）
...
- 主单元测试优先使用确定性证据（如 bounded repository read 计数、SQLite `EXPLAIN QUERY PLAN` / 索引命中）证明热路径；环境敏感的 wall-clock / p95 benchmark 应移出 release-blocking 单元套件或仅作为可选 benchmark
