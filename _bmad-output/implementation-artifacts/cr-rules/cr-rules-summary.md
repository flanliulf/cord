...
| ---------- | ------------------------------------------------------------------------- | ---------- | -------------- | ------------- | -------------- |
| CR-REPO-06 | 迁移子步骤必须独立幂等，并覆盖部分迁移数据库场景                          | 4-1        | 历史记录未量化 | global-doc    | 已同步全局文档 |
...
## Story 6-2 / 2026-05-19

- **Story**: 6-2
- **分析来源**:
  - `6-2-code-review-summary-20260519-round-1.md`
  - `6-2-code-review-evaluation-20260519-round-1.md`
  - `6-2-code-review-summary-20260519-round-2.md`
  - `6-2-code-review-evaluation-20260519-round-2.md`
- **结论概览**:
  - Round 1 暴露 2 个 P1 文档可读性问题：MCP outputSchema 表格和配置项表格中的联合类型未转义管道符，导致 GFM 表格列结构错乱。
  - Round 1 evaluation 确认两项问题均有效并已修复；修复后执行 Prettier check 与定向 Node 表格列数校验通过。
  - Round 2 reviewer / evaluator 确认历史阻塞项已关闭，未新增阻塞项、中高优先级问题或 CR TODO。
  - 本轮提炼出 1 条可复用的 Markdown 文档表格规则。按用户授权的保守默认策略仅 record-only 到 `cr-rules-summary.md`，不触发 Rule Document Registry 全局文档同步。

#### 升格判定摘要

| 候选规则                                                             | 硬性门槛 | 总分 | 建议去向      | 用户确认结果                      |
| -------------------------------------------------------------------- | -------- | ---- | ------------- | --------------------------------- |
| CR-DOC-01：Markdown 表格中的联合类型和枚举值必须转义管道符并校验列数 | 通过     | 8/12 | rules-summary | 已按默认决策写入 CR rules summary |

### 提炼规则

#### CR-DOC-01：Markdown 表格中的联合类型和枚举值必须转义管道符并校验列数

- **来源问题**: Story 6-2 的用户文档在 Markdown 表格单元格内直接书写联合类型或枚举值，例如 `critical | high | medium | low | info | none` 与 `Record<string, 'auto' | 'suggest' | 'log_only'>`，GFM 将未转义的 `|` 解析为列分隔符，导致 schema 与配置参考表格列结构错乱。
- **CR 证据**:
  - `6-2-code-review-summary-20260519-round-1.md`: reviewer 指出 `docs/mcp-tools-reference.md` 的 `AnalyzeImpactResult`、`QueryRelationsResult`、`SyncDocsResult` 表格以及 `docs/configuration.md` 的 `updateStrategies` 行被未转义管道符拆裂。
  - `6-2-code-review-evaluation-20260519-round-1.md`: evaluator 确认两项发现有效，均按 P1 阻塞交付处理，并记录已采用转义管道符的最小修复。
  - `6-2-code-review-summary-20260519-round-2.md`: Round 2 确认目标表格列数校验通过，历史问题无回归。
  - `6-2-code-review-evaluation-20260519-round-2.md`: Round 2 evaluator 确认两项历史阻塞项已关闭，且无新增 CR TODO。
- **硬性门槛**:
  - 有证据: 是
  - 可规则化: 是
  - 非纯特例: 是
...
- **量化评分**:
  ...
  | 维度       | 分数 | 理由                                                                                         |
  | ---------- | ---- | -------------------------------------------------------------------------------------------- |
  | 复现频次   | 1    | 同一 Story 内跨 MCP 工具参考与配置参考两个文档、多张表格重复出现同一 Markdown 表格解析问题。 |
  | 影响范围   | 1    | 主要影响用户文档、schema 表格和配置参考，属于文档技术域而非运行时代码。                      |
  | 风险等级   | 1    | 不改变运行时行为，但会破坏 Story 文档验收和读者对 outputSchema / 配置值域的理解。            |
  | 根因稳定性 | 1    | 根因是 Markdown 表格内联合类型书写习惯与 Prettier 不能验证渲染语义，后续文档更新时可能复发。 |
  | 可执行性   | 2    | 可明确要求表格单元格内的联合类型分隔符必须转义，并用表格列数校验或渲染检查作为回归信号。     |
  | 文档缺口   | 2    | 现有全局规则和 CR rules summary 未覆盖 Markdown 表格中的联合类型 / 枚举值渲染检查。          |

- **总分**: 8/12
- **建议去向**: rules-summary
- **适用范围**: 用户文档、MCP / CLI schema 文档、配置参考、任何在 Markdown 表格单元格中描述联合类型或枚举值的文档。
- **规避指南**:
  - 禁止在 Markdown 表格单元格内直接书写未转义的联合类型分隔符 `|`。
  - 禁止只依赖 Prettier 判断 Markdown 表格语义正确；Prettier 可能格式化出列数一致的错误结构。
- **最佳实践**:
  - 表格内需要展示联合类型或枚举值时，将 `|` 写作 `\|`；若类型过长，则改用命名类型并在表格外用列表说明允许值。
  - 对包含 schema / 配置项表格的用户文档，至少执行一次表格列数校验或渲染检查，确认每行列数与表头一致。
- **全局文档建议**:
  - 暂不升格到 Rule Document Registry 文档。该规则可复用，但当前证据集中在 Story 6-2 的文档编写场景，且属于文档技术域的局部质量规则；若后续 Story 再次出现 Markdown 表格渲染语义缺陷，再考虑同步写入 `_bmad-output/project-context.md`、`03-core-architectural-decisions.md`、`04-implementation-patterns-consistency-rules.md` 的文档质量或验收规则章节。
- **本次落地**:
  - 已在 Round 1 修复执行记录中落地到 `docs/mcp-tools-reference.md` 与 `docs/configuration.md`，并通过 Prettier check 与定向表格列数校验。
  - 已按默认决策记录到 `cr-rules-summary.md`；不修改源码、Story 文档或全局规则文档。
- **同步状态**: 已写入规则总结

### 治理结论

- Story 6-2 本轮新增 1 条可复用文档规则，按保守默认决策 record-only 到 `cr-rules-summary.md`。
- 本轮不更新 `_bmad-output/project-context.md`、`03-core-architectural-decisions.md`、`04-implementation-patterns-consistency-rules.md`，因为该规则尚未达到必须全局升格的范围。
